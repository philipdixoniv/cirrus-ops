...
def get_client() -> Client:
    """Create and return a Supabase admin client (service-role key)."""
...
def client() -> Client:
    """Return the singleton admin Supabase client (service-role key, bypasses RLS)."""
    global _client
    if _client is None:
        _client = get_client()
    return _client
...
def user_client(jwt: str) -> Client:
    """Create a Supabase client using a user's JWT (RLS enforced)."""
...
def upsert_meeting(data: dict[str, Any]) -> dict[str, Any]:
    ...
    data must include org_id for the unique constraint.
    """
    result = (
        client()
        .table("meetings")
        .upsert(data, on_conflict="org_id,platform,external_id")
        .execute()
    )
...
def count_meetings(org_id: str | None = None, platform: str | None = None) -> int:
    """Count meetings, optionally filtered by org and platform."""
...
def get_sync_state(platform: str, org_id: str | None = None) -> dict[str, Any] | None:
    """Get the current sync state for a platform (org-scoped)."""
    query = client().table("sync_state").select("*").eq("platform", platform)
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
    return result.data[0] if result.data else None
...
def update_sync_state(platform: str, org_id: str | None = None, **kwargs: Any) -> None:
    """Update sync state fields for a platform (org-scoped)."""
    query = client().table("sync_state").update(kwargs).eq("platform", platform)
    if org_id:
        query = query.eq("org_id", org_id)
    query.execute()
...
def get_profile(name: str, org_id: str | None = None) -> dict[str, Any] | None:
    """Fetch a mining profile by name, optionally scoped by org."""
    query = client().table("mining_profiles").select("*").eq("name", name)
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
    return result.data[0] if result.data else None
...
def list_profiles(org_id: str | None = None) -> list[dict[str, Any]]:
    """List mining profiles, optionally scoped by org."""
    query = client().table("mining_profiles").select("*")
    if org_id:
        query = query.eq("org_id", org_id)
...
def search_stories(query: str, org_id: str | None = None, limit: int = 20, offset: int = 0) -> tuple[list[dict[str, Any]], int]:
...
def search_content(query: str, org_id: str | None = None, limit: int = 20, offset: int = 0) -> tuple[list[dict[str, Any]], int]:
...
def get_theme_counts(org_id: str | None = None) -> list[dict[str, Any]]:
    """Get all unique themes with story counts via RPC or manual aggregation."""
    query = client().table("extracted_stories").select("themes")
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
    counts: dict[str, int] = {}
...
def get_themes_over_time(months: int = 12, org_id: str | None = None) -> list[dict[str, Any]]:
    """Get theme counts grouped by month for the last N months."""
    query = client().table("extracted_stories").select("themes, created_at")
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.order("created_at", desc=True).execute()
...
def get_sentiment_breakdown(profile_id: str | None = None, org_id: str | None = None) -> list[dict[str, Any]]:
...
def get_top_companies(limit: int = 10, org_id: str | None = None) -> list[dict[str, Any]]:
    """Get most-mentioned customer companies."""
    query = client().table("extracted_stories").select("customer_company").not_.is_("customer_company", "null")
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
    counts: dict[str, int] = {}
...
def get_content_pipeline(profile_id: str | None = None, org_id: str | None = None) -> list[dict[str, Any]]:
...
def count_stories(org_id: str | None = None) -> int:
    """Count total extracted stories."""
    query = client().table("extracted_stories").select("id", count="exact")
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
    return result.count or 0
...
def count_content(org_id: str | None = None) -> int:
    """Count total generated content."""
    query = client().table("generated_content").select("id", count="exact")
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
    return result.count or 0
...
def list_presets(profile_id: str, org_id: str | None = None) -> list[dict[str, Any]]:
...
def get_competitor_mentions(limit: int = 20, org_id: str | None = None) -> list[dict[str, Any]]:
    """Scan story_text for known competitor names, aggregate counts."""
    query = client().table("extracted_stories").select("id, story_text")
    if org_id:
        query = query.eq("org_id", org_id)
    result = query.execute()
...
def get_recent_activity(limit: int = 15, org_id: str | None = None) -> list[dict[str, Any]]:
    """Get recent stories + content ordered by created_at."""
    sq = client().table("extracted_stories").select("id, title, created_at")
    cq = client().table("generated_content").select("id, content_type, status, story_id, created_at")
    if org_id:
        sq = sq.eq("org_id", org_id)
        cq = cq.eq("org_id", org_id)
    stories_result = sq.order("created_at", desc=True).limit(limit).execute()
    content_result = cq.order("created_at", desc=True).limit(limit).execute()
